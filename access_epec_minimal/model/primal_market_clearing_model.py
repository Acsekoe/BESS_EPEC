...
class MarketData:
    ...
    eta: float
    demand_adjustment_penalty_eur_per_mw2: float = 500.0
    # Submitted market offers used by the ISO dispatch objective.  When None
    # (or when a generator is absent), the true economic cost is offered, so
    # every historical input remains a truthful benchmark.
    generation_offer: Mapping[str, float] | None = None


def effective_generation_offer(data: "MarketData", generator: str) -> float:
    """Submitted ISO offer for one generator: explicit offer, else true cost."""

    if data.generation_offer is not None and generator in data.generation_offer:
        return float(data.generation_offer[generator])
    return float(data.generation_cost[generator])
...
def _validate_data(data: MarketData) -> None:
    ...
    if data.voll < 0.0:
        raise ValueError("voll must be non-negative.")
    if data.demand_adjustment_penalty_eur_per_mw2 <= 0.0:
        raise ValueError("Demand-adjustment penalty must be positive.")
    if data.generation_offer is not None:
        unknown_offers = set(data.generation_offer) - set(data.generators)
        if unknown_offers:
            raise ValueError(f"generation_offer has unknown generators: {sorted(unknown_offers)}")
    for n in data.nodes:
...
def build_primal_market_clearing_model(
    ...
    def objective_rule(model: pyo.ConcreteModel) -> pyo.Expression:
        # The ISO clears against submitted offers, not true economic costs.
        generation_cost = sum(
            effective_generation_offer(data, g) * model.P_gen[g, t]
            for g in model.G
            for t in model.T
        )
        load_shed_cost = (
        ...
        price_regularization = 0.5 * data.demand_adjustment_penalty_eur_per_mw2 * sum(
            model.DemandAdjustment[n, t] ** 2 for n in model.N for t in model.T
        )
        return generation_cost + load_shed_cost + price_regularization
