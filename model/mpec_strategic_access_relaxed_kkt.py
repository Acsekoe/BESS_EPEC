...
capacity.  The ISO lower level jointly awards connection MW and clears the
physical 24-hour market.  Awarded MW, dispatch, SOC, and all network variables
therefore belong to the lower level; energy investment remains upper-level.

For fixed strategies the ISO problem is a linear program.  Its optimality is
embedded with primal feasibility, dual feasibility/stationarity, and one
...
def build_fixed_access_market(
    ...
    *,
    ...
    """Build the exact ISO LP for one fixed access-strategy profile."""
...
def solve_fixed_access_market(
    ...
    *,
    ...
    """Solve and return the exact fixed-strategy access/dispatch LP."""
...
def build_model(
    ...
    **_: object,
    ...
    if epsilon < 0.0 or proximal_penalty < 0.0:
        raise ValueError("Complementarity epsilon and proximal penalty cannot be negative.")
    ...
    initial_bid_values = {
        node: min(
            access_bid_bound,
            max(0.0, float((initial_access_bid or {}).get(node, 0.0))),
        )
        for node in data.nodes
    }
    initial_energy = {
    ...
    model.access_payment = pyo.Expression(
        expr=sum(
            model.AccessBid[n] * model.X_awarded[active, n] for n in model.N
        )
    )
    ...
    if proximal_penalty > 0.0:
        ...
        for stem in ("quantity", "bid", "energy"):
            model.add_component(
                f"{stem}_dev_pos",
                pyo.Var(model.N, domain=pyo.NonNegativeReals, initialize=0.0),
            )
            model.add_component(
                f"{stem}_dev_neg",
                pyo.Var(model.N, domain=pyo.NonNegativeReals, initialize=0.0),
            )
        model.quantity_deviation = pyo.Constraint(
            model.N,
            rule=lambda m, n: m.AccessQuantity[n] - float(proximal_access_quantity[n])
            == m.quantity_dev_pos[n] - m.quantity_dev_neg[n],
        )
        model.bid_deviation = pyo.Constraint(
            model.N,
            rule=lambda m, n: m.AccessBid[n] - float(proximal_access_bid[n])
            == m.bid_dev_pos[n] - m.bid_dev_neg[n],
        )
        model.energy_deviation = pyo.Constraint(
            model.N,
            rule=lambda m, n: m.EnergyCapacity[n] - float(proximal_energy_capacity[n])
            == m.energy_dev_pos[n] - m.energy_dev_neg[n],
        )
        regularizer = proximal_penalty * sum(
            model.quantity_dev_pos[n]
            + model.quantity_dev_neg[n]
            + (model.bid_dev_pos[n] + model.bid_dev_neg[n]) / proximal_price_scale
            + (model.energy_dev_pos[n] + model.energy_dev_neg[n])
            / proximal_energy_scale
            for n in model.N
        )
    else:
    ...
    model._access_settlement = "pay-as-bid"
...
def initialise_lower_level(model: pyo.ConcreteModel, data: MarketData) -> None:
    ...
    if hasattr(model, "quantity_dev_pos"):
        for node in model.N:
            differences = {
                "quantity": float(pyo.value(model.AccessQuantity[node]))
                - float(model._proximal_access_quantity[node]),
                "bid": float(pyo.value(model.AccessBid[node]))
                - float(model._proximal_access_bid[node]),
                "energy": float(pyo.value(model.EnergyCapacity[node]))
                - float(model._proximal_energy_capacity[node]),
            }
            for stem, difference in differences.items():
                getattr(model, f"{stem}_dev_pos")[node].set_value(max(0.0, difference))
                getattr(model, f"{stem}_dev_neg")[node].set_value(max(0.0, -difference))
