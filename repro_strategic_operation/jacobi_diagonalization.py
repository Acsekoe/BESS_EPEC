...
class JacobiConfig:
    ...
    proximal_unpenalized_sweeps: int = 10
...
class JacobiResult:
    ...
    nodal_history: list[dict[str, object]] = field(default_factory=list)
...
def _validate(config: JacobiConfig) -> None:
    ...
    if (
        ...
        or config.proximal_unpenalized_sweeps < 0
    ):
        raise ValueError(
            "Maximum/convergence sweep counts must be positive and startup "
            "sweep counts cannot be negative."
        )
    ...
    if (
        ...
        or config.proximal_quantity_scale <= 0.0
        or config.proximal_energy_scale <= 0.0
    ):
        raise ValueError("Price/dual bounds and proximal scales must be positive.")
    if (
        config.formulation
        in {
            "strategic-price-quantity",
    ...
    if (
        ...
        or config.access_bid_slope_eur_per_mw2_day < 0.0
        or config.access_bid_slope_eur_per_mw2_day
        * min(config.node_limit_mw, config.access_request_limit_mw)
        > config.access_bid_bound
...
def run_jacobi(
    ...
    for sweep in range(state.sweep + 1, config.max_sweeps + 1):
        effective_proximal_penalty = (
            0.0
            if sweep <= config.proximal_unpenalized_sweeps
            else config.proximal_penalty
        )
        sweep_config = replace(
            config, proximal_penalty=effective_proximal_penalty
        )
        effective_damping = (
            1.0
            ...
            state.history.append(
                {
                    ...
                    "proximal_penalty": effective_proximal_penalty,
                    "configured_proximal_penalty": config.proximal_penalty,
                    "proximal_unpenalized_sweeps": (
                        config.proximal_unpenalized_sweeps
                    ),
        ...
        if access_strategic:
            slope = config.access_bid_slope_eur_per_mw2_day
            for investor in config.investors:
                investor_id = investor.investor_id
                for node in data.nodes:
                    key = investor_id, node
                    state.nodal_history.append(
                        {
                            "sweep": sweep,
                            "investor": investor_id,
                            "node": node,
                            "old_request_mw": old_access_quantity[key],
                            "best_response_request_mw": (
                                proposals_access_quantity[key]
                            ),
                            "new_request_mw": state.access_quantity[key],
                            "old_bid_eur_per_mw_day": old_access_bid[key],
                            "best_response_bid_eur_per_mw_day": (
                                proposals_access_bid[key]
                            ),
                            "new_bid_eur_per_mw_day": state.access_bid[key],
                            "old_award_mw": old_power[key],
                            "best_response_award_mw": proposals_power[key],
                            "new_award_mw": state.power[key],
                            "old_energy_mwh": old_energy[key],
                            "best_response_energy_mwh": proposals_energy[key],
                            "new_energy_mwh": state.energy[key],
                            "marginal_bid_at_award_eur_per_mw_day": (
                                state.access_bid[key] - slope * state.power[key]
                            ),
                            "integrated_access_payment_eur_per_day": (
                                state.access_bid[key] * state.power[key]
                                - 0.5 * slope * state.power[key] ** 2
                            ),
                        }
                    )
