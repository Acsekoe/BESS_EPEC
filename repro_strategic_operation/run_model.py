...
ACCESS_FORMULATIONS = frozenset({"strategic-access"})
ACCESS_SETTLEMENT = "integrated-linear-pay-as-bid"
...
def _resume_signature(config: JacobiConfig, data_sha256: str) -> dict[str, object]:
    signature: dict[str, object] = {
        ...
        "proximal_unpenalized_sweeps": config.proximal_unpenalized_sweeps,
        ...
        signature.update(
            {
                ...
                "access_bid_slope_eur_per_mw2_day": (
                    config.access_bid_slope_eur_per_mw2_day
                ),
                ...
                "proximal_price_scale": config.proximal_price_scale,
                "proximal_quantity_scale": config.proximal_quantity_scale,
                "proximal_regularizer": "scaled-quadratic",
                "access_settlement": "integrated-linear-pay-as-bid",
            }
        )
    if config.formulation in {
...
def _checkpoint(
    ...
    if access_strategic:
        ...
        payload["nodal_history"] = state.nodal_history
...
def _load_checkpoint(
    ...
    if access_strategic and isinstance(actual_signature, dict):
        if "access_bid_slope_eur_per_mw2_day" not in actual_signature:
            actual_signature = {
                **actual_signature,
                "access_bid_slope_eur_per_mw2_day": 0.0,
            }
        if actual_signature.get("access_settlement") == "pay-as-bid":
            actual_signature = {
                **actual_signature,
                "access_settlement": "integrated-linear-pay-as-bid",
            }
    if (
        ...
        and "proximal_unpenalized_sweeps" not in actual_signature
    ):
        actual_signature = {
            **actual_signature,
            "proximal_unpenalized_sweeps": 0,
        }
    ...
    if access_strategic:
        ...
        for investor_id in investor_ids:
            ...
            for node in data.nodes:
                ...
                if (
                    access_bid[key] + tolerance
                    < config.access_bid_slope_eur_per_mw2_day
                    * access_quantity[key]
                ):
                    raise ValueError(
                        f"Checkpoint access bid has a negative marginal end bid at {key}."
                    )
    ...
    nodal_history = payload.get("nodal_history", []) if access_strategic else []
    if not isinstance(nodal_history, list) or any(
        not isinstance(row, dict) for row in nodal_history
    ):
        raise ValueError("Checkpoint nodal history must be a list of records.")
    for row in nodal_history:
        row.setdefault("common_access_price_eur_per_mw_day", None)
        row.setdefault(
            "settled_access_payment_eur_per_day",
            row.get("integrated_access_payment_eur_per_day"),
        )
        row.setdefault("access_settlement", ACCESS_SETTLEMENT)
...
def _write_outputs(
    ...
    if state.nodal_history:
        with (output_dir / "nodal_history.csv").open(
            "w", newline="", encoding="utf-8"
        ) as handle:
            writer = csv.DictWriter(
                handle, fieldnames=list(state.nodal_history[0])
            )
            writer.writeheader()
            writer.writerows(state.nodal_history)
    ...
    access_lower = None
    if config.formulation in ACCESS_FORMULATIONS:
        ...
        def integrated_access_payment(key: tuple[str, str]) -> float:
            return (
        ...
        def settled_access_payment(key: tuple[str, str]) -> float:
            return integrated_access_payment(key)
        ...
        with (output_dir / "final_access_bids.csv").open(
            ...
            writer.writerow(
                [
                    ...
                    "marginal_bid_at_award_eur_per_mw_day",
                    "integrated_access_payment_eur_per_day",
                    "common_access_price_eur_per_mw_day",
                    "settled_access_payment_eur_per_day",
                    "access_settlement",
                ]
            )
                    ...
                    writer.writerow(
                        [
                            *key,
                            ...
                            - config.access_bid_slope_eur_per_mw2_day
                            * state.power[key],
                            integrated_access_payment(key),
                            common_access_price[node],
                            settled_access_payment(key),
                            ACCESS_SETTLEMENT,
                        ]
                    )
        with (output_dir / "final_nodal_access.csv").open(
            ...
            writer.writerow(
                [
                    ...
                    "settlement_price_eur_per_mw_day",
                ]
            )
            for node in data.nodes:
                writer.writerow(
    ...
    summary = {
        ...
        "projection_count": state.projection_count,
        "stable_sweeps": state.stable_sweeps,
        "configured_proximal_penalty": config.proximal_penalty,
        "proximal_unpenalized_sweeps": config.proximal_unpenalized_sweeps,
        "proximal_activation_sweep": config.proximal_unpenalized_sweeps + 1,
    ...
    if config.formulation in ACCESS_FORMULATIONS:
        ...
        summary["strategic_access"] = {
            ...
            "proximal_regularizer": "scaled-quadratic",
            "proximal_quantity_scale_mw": config.proximal_quantity_scale,
            "proximal_energy_scale_mwh": config.proximal_energy_scale,
            "proximal_bid_scale_eur_per_mw_day": config.proximal_price_scale,
            ...
            "investors": {
                investor.investor_id: {
                    ...
                    "access_payment_eur_per_day": sum(
                        settled_access_payment((investor.investor_id, node))
                        for node in data.nodes
                    ),
            ...
            "nodal_access": {
                node: {
                    ...
                    "settlement_price_eur_per_mw_day": None,
                }
                for node in data.nodes
            },
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Best-response regularizer coefficient; strategic-access uses a "
            "scaled quadratic penalty, while other formulations retain their "
            "existing regularizer. Zero is the baseline."
        ),
    )
    parser.add_argument(
        "--proximal-unpenalized-sweeps",
        type=int,
        default=10,
        help=(
            "Number of initial sweeps with rho forced to zero; the configured "
            "--proximal-penalty activates on the following sweep."
        ),
    )
    parser.add_argument(
        "--proximal-quantity-scale",
        type=float,
        default=1.0,
        help="MW normalization for strategic-access quadratic request changes.",
    )
    ...
    parser.add_argument(
        ...
        default=10.0,
        help=(
            "Price normalization: EUR/MW-day for strategic access and EUR/MWh "
            "for operational bidding formulations."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Fixed downward slope of the marginal access bid in "
            "EUR/(MW^2 day); zero retains a flat access bid."
        ),
    )
    parser.add_argument(
        "--initial-access-bid",
...
def main() -> int:
    ...
    config = JacobiConfig(
        ...
        proximal_unpenalized_sweeps=args.proximal_unpenalized_sweeps,
        proximal_quantity_scale=args.proximal_quantity_scale,
        ...
        access_bid_slope_eur_per_mw2_day=args.access_bid_slope,
    ...
    def after_sweep(state: JacobiResult) -> None:
        ...
        bid_status = (
            ...
            if args.formulation in ACCESS_FORMULATIONS
            else " / "
    ...
    if args.formulation in ACCESS_FORMULATIONS and args.access_undamped_sweeps > 0:
        damping_status = (
    ...
    proximal_status = (
        "rho=0"
        if args.proximal_penalty == 0.0
        else (
            f"rho=0 through sweep {args.proximal_unpenalized_sweeps}, "
            f"then rho={args.proximal_penalty:g}"
        )
    )
    print(
        f"Running {len(investors)}-investor Jacobi with {args.formulation}; "
        f"{damping_status}, {proximal_status}, max_sweeps={args.max_sweeps}, "
